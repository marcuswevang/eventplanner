...
export default function BudgetManager({
    ...
    const handleUpdateGoal = async () => {
        ...
        const oldGoal = budgetGoal;
        setBudgetGoal(goal);
        setIsEditingGoal(false);
        const result = await updateEventSettings(eventId, { budgetGoal: goal } as any);
        if (!result.success) {
            alert("Kunne ikke oppdatere budsjettmål: " + (result.error || "Ukjent feil"));
            setBudgetGoal(oldGoal);
        }
    };
    ...
    const handleTogglePaid = async (item: BudgetItem) => {
        ...
        const updates: any = { isPaid: newPaidStatus };

        // Autofill actual cost with estimated if marking as paid and it's empty
        if (newPaidStatus && (!item.actualCost || item.actualCost === 0)) {
            updates.actualCost = item.estimatedCost;
        }

        const result = await updateBudgetItem(item.id, updates);
        if (result.success) {
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={`${styles.mainList} glass`} style={{ overflowX: 'auto' }}>
